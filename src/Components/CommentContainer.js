import React from "react";

// Dummy Comment Data
const commentData = [
  {
    name: "Abhishek Singh",
    text: "jdslakjflkjddlfkjl;kj;glkjglkdfgjf",
    replies: [],
  },
  {
    name: "Abhishek Singh",
    text: "jdslakjflkjddlfkjl;kj;glkjglkdfgjf",
    replies: [
      {
        name: "Abhishek Singh",
        text: "jdslakjflkjddlfkjl;kj;glkjglkdfgjf",
        replies: [
          {
            name: "Abhishek Singh",
            text: "jdslakjflkjddlfkjl;kj;glkjglkdfgjf",
            replies: [
              {
                name: "Abhishek Singh",
                text: "jdslakjflkjddlfkjl;kj;glkjglkdfgjf",
                replies: [
                  {
                    name: "Abhishek Singh",
                    text: "jdslakjflkjddlfkjl;kj;glkjglkdfgjf",
                    replies: [
                      {
                        name: "Abhishek Singh",
                        text: "jdslakjflkjddlfkjl;kj;glkjglkdfgjf",
                        replies: [
                          {
                            name: "Abhishek Singh",
                            text: "jdslakjflkjddlfkjl;kj;glkjglkdfgjf",
                            replies: [
                              {
                                name: "Abhishek Singh",
                                text: "jdslakjflkjddlfkjl;kj;glkjglkdfgjf",
                                replies: [],
                              },
                            ],
                          },
                        ],
                      },
                    ],
                  },
                ],
              },
            ],
          },
        ],
      },
    ],
  },
  {
    name: "Abhishek Singh",
    text: "jdslakjflkjddlfkjl;kj;glkjglkdfgjf",
    replies: [
      {
        name: "Abhishek Singh",
        text: "jdslakjflkjddlfkjl;kj;glkjglkdfgjf",
        replies: [],
      },
    ],
  },
  {
    name: "Abhishek Singh",
    text: "jdslakjflkjddlfkjl;kj;glkjglkdfgjf",
    replies: [],
  },
];

// Comment compponent for each comment UI represtation
const Comment = ({ data }) => {
  const { name, text, replies } = data;
  return (
    <div className="flex mx-4 my shadow-md rounded-lg">
      <img
        className="h-11 w-12"
        src="data:image/png;base64,"
        alt="User_logo"
      />
      <div className="px-2">
        <h3 className="font-extrabold">{name}</h3>
        <h4>{text}</h4>
        {replies.length != 0 ? <h1 className="px-4 font-bold">Reply</h1> : ""}
      </div>
    </div>
  );
};

// Map all the componnennt
const CommentList = ({ allComments }) =>
  allComments.map((eachCommet, index) => {
    return (
      // Do not use index as a key
      <div key={index}>
        <Comment data={eachCommet} />
        {/* Handle reply usinng recursion */}
        <div className="ml-3 px-3 border border-l-black">
          {/* Below code did not throw error bcz map function hanndle empty array, and do work on it and return an empty array. */}
          <CommentList allComments={eachCommet.replies} />
          {/* {eachCommet.replies.length != 0 && (
          <CommentList allComments={eachCommet.replies} />
        )} */}
        </div>
      </div>
    );
  });

function CommentContainer() {
  return (
    <div>
      {" "}
      <h1 className="font-bold p-1 m-1 ">Comment :</h1>
      {/* <Comment data={commentData[1]} /> */}
      <CommentList allComments={commentData} />
    </div>
  );
}

export default CommentContainer;
